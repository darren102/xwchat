...
var hasFocus = false, windowHasFocus = false;
...
function xcMsgGet(m) {
  ...
  // verify whether we have a delayed delivery alias and hence that should be utilized instead
  if (JQ(m.getDoc()).find('delay').size() > 0) {
    JQ(m.getDoc()).find('delay').each(function() {
      if (JQ(this).attr('xmlns') == 'urn:xmpp:delay' && JQ(this).attr('from') != 'undefined') {
        alias = JQ(this).attr('from');
      };
    });
  }
  ...
  JQ('div.xcMUCOccupant:not(.xcMUCCurrentUserContainer)').unbind().mousedown(function(e) {
    ...
    JQ('.xcContextMenu > ul > li').unbind().click(function() {
      if (this.id == 'ban') {
        ...
        var w = pWin.xcOpenUserChat(id); // open a one on one chat session with the user
...
JQ(document).ready(function() {
  ...
  // If focus is on the searchjid
  JQ('#searchjid').focus(function() {
    hasFocus = true;
  }).blur(function() {
    hasFocus = false;
    setTimeout(function() {
      if (hasFocus === false && windowHasFocus === true) {
        JQ('#mbody').focus();
        JQ('#mbody').val(JQ('#mbody').val());
      }
    }, 500);
  });
  // If focus is on the invite message
  JQ('#imessage').focus(function() {
    hasFocus = true;
  }).blur(function() {
    hasFocus = false;
    setTimeout(function() {
  ...
  // If focus is on the subject change text box
  JQ('#subject').focus(function() {
    hasFocus = true;
  }).blur(function() {
    hasFocus = false;
    setTimeout(function() {
  ...
  // putting focus onto the body
  JQ('#mbody').blur(function() {
    setTimeout(function() {
      if (hasFocus === false && windowHasFocus === true) {
        JQ('#mbody').focus();
        JQ('#mbody').val(JQ('#mbody').val());
