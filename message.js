...
function xcMsgChatHistoryToMUC(room) {
  ...
  JQ('div#msg_pane div.xcChatMessage p.xcChatMessageBody').each(function() {
    ...
    if (body) {
      if (JQ(this).parent('div.xcChatMessage').hasClass('received') === true) {
        pWin.xcMsgSendChatHistory(body, room, 'groupchat', tabObject[activeTab].jid);
      } else {
        pWin.xcMsgSendChatHistory(body, room, 'groupchat', null);
      }
    }
  });
}
...
function xcMsgSend() {
  ...
  if (body.replace(/ /g, '').replace(/\r\n/g, '').replace(/\n/g, '').replace(/\r/g, '').length == 0) {
    JQ('#body').val('').focus(); // clear the message box for the next message and reset focus back onto it
    return false;
  };
